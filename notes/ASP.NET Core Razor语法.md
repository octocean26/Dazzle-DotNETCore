...
在视图的上下文中，通过C＃块引用视图组件，并将所需的任何输入数据传递给它们。在内部，视图组件将运行自己的逻辑，处理传入的数据，并返回准备呈现的视图。

与Tag助手不同，ASP.NET Core没有任何预定义的视图组件。因此，视图组件是基于每个应用程序创建的。
...
在视图组件类中，您可能具有数据库或服务引用，并可能要求系统为您注入依赖项。这是一种完全不同的业务逻辑，它从数据所在的位置获取数据，并将其打包成HTML块。
...
请注意，视图组件也可以由控制器调用。在这种情况下，使用的代码与下面所示的代码类似：
...
这种方法类似于返回部分视图。在这两种情况下，调用者都将收到一个HTML片段。部分视图和视图组件之间的区别在于它们的内部实现。部分视图是一个普通的Razor模板，它接收并合并模板中的数据。视图组件接收输入参数，检索其数据，然后将其合并到模板中。

视图组件的作用是帮助对视图进行组件化，使其由独立的和自制的小部件组合而成。 

#### 聚合数据和UI模板

应用程序中的某些视图应该是来自不同查询的数据聚合的理想结果。在此上下文中，查询不一定是数据库查询，但它是一个返回视图所需的数据的操作。您可以定义一个规范的视图模型对象，并让应用程序控制器使用它从多个操作（可能是并行操作）中获取的数据进行填充。
...
对于上述模型对象，通常，应用程序服务将触发三次调用以获取数据。
...
在这种方法中，数据的检索是集中的。视图可能由三个不同的部分视图组成，每个部分视图接收一个数据块。
...
另一种方法是将这个视图拆分成三个较小的、独立的部分，每个部分专用于一个查询任务。视图组件只是部分视图加上一些专用的查询逻辑。然后可以表达相同的视图，如下所示。
...
负责呈现视图的控制器不需要通过应用程序服务，它可以只呈现视图，然后渲染视图将触发组件。

### 视图组件与子Actions

视图组件看起来非常类似于经典ASP.NET MVC的部分视图和子Action。 ASP.NET Core具有部分视图但缺少子操作。与子操作相比，视图组件更快，因为它不像子操作那样通过控制器管道。特别是，这意味着没有模型绑定，也没有动作过滤器。

视图组件如何与部分视图进行比较？

部分视图只是接收和呈现数据的模板，他们没有后端逻辑。它们通常没有代码，或者只包含一些渲染逻辑。视图组件通常查询某个数据库以获取其数据。

#### 视图组件的影响

将视图拆分为独立组件主要是一种方便的工作组织方式，将视图分割为独立组件还可以让不同的开发人员处理不同的部分，从而使创建视图的过程更加并行。然而，视图组件不一定是加速应用程序的方法。

 因为每个视图组件都是独立呈现的(并随后进行填充)，所以数据库逻辑可能没有得到优化。除非数据位于不同且不相关的数据源中，否则多个独立查询可能涉及多个连接和多个命令。

作为一般规则，确保不会通过将视图分割为组件来破坏最终的整体查询逻辑。下面是一个例子。假设一个站点的主页必须呈现一个框，其中包含三个最近的新闻标题，另一个框中包含最后十个新闻，其中有照片、标题和摘要。不同的视图组件需要对同一个数据库表进行两个不同的查询。相反，集中的数据检索过程可能只需要一个查询。
